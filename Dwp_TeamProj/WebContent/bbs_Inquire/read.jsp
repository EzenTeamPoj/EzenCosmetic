...
//out.print("numParam(read) :" + numParam);
// 검색어 수신 시작
...
// 현재 페이지 돌아가기 소스 끝
lMgr.upCountInq(numParam); // 조회수 증가
...
String fileName = bean.getOriFileName();
	...
	<div id="wrap">
		...
		<main id="main" class="dFlex">
			...
			<div id="contents" class="bbsRead">
					...
					<tfoot id="readTblFoot">
						...
						<tr>
							...
							<td colspan="4" id="btnAreaTd" class="read">
								<button type="button" id="listBtn"><%=listBtnLabel %> 
								</button>
								<%
								if(aId_Session!=null){%>
								<button type="button" id="inqReplyBtn">답 변</button> <%
								}%>
								...
								<%
										...
										<button type="button" id="delInqBtn">삭 제</button>
										<% } 
				...
				<input type="hidden" name="nowPageInq" value="<%=nowPage%>"
					id="nowPageInq"> 
				...
				<!-- 검색어전송 끝 -->
<input type="hidden" id="uId_Session" value="<%=uId_Session%>"> 
